...
async function checkLessonOwnership(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to validate lesson access';
...
export async function GET(
  ...
  const ownershipCheck = await checkLessonOwnership(request, courseId, lessonId);
...
export async function PATCH(
  ...
  try {
    ...
    // Update the lesson
    const updatedLesson = await db.update(courseSchema.lesson)
      .set({
        ...lessonData,
        updatedAt: new Date()
      })
