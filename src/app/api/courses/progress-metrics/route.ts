...
export async function GET(request: NextRequest) {
  try {
      ...
      courses.map(async (course) => {
        ...
        const dropoutResult = await db
          ...
          .leftJoin(
            ...
            and(
              eq(courseSchema.courseEnrollment.courseId, courseSchema.lessonProgress.courseId),
              eq(courseSchema.courseEnrollment.userId, courseSchema.lessonProgress.userId)
            )
          )
    ...
    const message = error instanceof Error ? error.message : 'Failed to fetch progress metrics';
    return NextResponse.json(
      { error: message },
      { status: 500 }
    );
