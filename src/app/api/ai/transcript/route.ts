...
import { db, lesson as lessonTable } from '@/db';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { transcript: vttContent, error: transcriptError } = await fetchMuxTranscript(playbackId);
    ...
    const structuredTranscript = segments.map(seg => 
...
`;
      ...
      const finalChapters = chapters.map((chapter: ChapterData, index: number) => ({
        ...chapter,
        id: (index + 1).toString()
      }));
