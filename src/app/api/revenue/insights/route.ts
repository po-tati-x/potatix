...
export async function GET(request: NextRequest) {
  try {
    ...
    if (previousMonthRevenue > 0) {
      momRevenueChange = Math.round(((currentMonthRevenue - previousMonthRevenue) / previousMonthRevenue) * 100);
    ...
    const avgCourseValue = courses.length > 0 
    ...
    const courseRevenueData = await Promise.all(
      courses.map(async (course) => {
        ...
        const lastMonthEnrollment = previousMonthEnrollments
          .find(e => e.courseId === course.id);
        const thisMonthEnrollment = currentMonthEnrollments
          .find(e => e.courseId === course.id);
        
        const lastMonthCount = lastMonthEnrollment ? lastMonthEnrollment.count : 0;
        const thisMonthCount = thisMonthEnrollment ? thisMonthEnrollment.count : 0;
    ...
    const message = error instanceof Error ? error.message : 'Failed to fetch revenue insights';
    return NextResponse.json(
      { error: message },
      { status: 500 }
    );
