...
async function getModuleWithAuth(
  request: NextRequest,
  courseId: string,
  moduleId: string,
) {
...
export async function DELETE(
  ...
  try {
    ...
    console.log(
      `[API:DELETE:Module] Found ${lessonsWithVideos.length} lessons with potential videos to clean up`,
    );
    ...
    for (const lesson of lessonsWithVideos) {
      if (lesson.videoId) {
        console.log(
          `[API:DELETE:Module] Cleaning up Mux asset for lesson ${lesson.id} with playback ID ${lesson.videoId}`,
        );
        ...
        if (assetId) {
          ...
          console.log(
            `[API:DELETE:Module] Mux asset deletion for ${assetId}: ${deleted ? "Success" : "Failed"}`,
          );
        }
      }
    }
