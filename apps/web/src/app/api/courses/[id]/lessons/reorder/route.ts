...
export async function PATCH(
  ...
  console.log(
    `[DEBUG] PATCH /api/courses/${courseId}/lessons/reorder - Starting...`,
  );
  ...
  try {
    ...
    console.log(
      `[DEBUG] Processing ${lessons.length} lessons for reordering:`,
      lessons,
    );
    ...
    console.log(
      `[DEBUG] Found ${existingLessons.length} lessons in database:`,
      existingLessons,
    );
    ...
    await db.transaction(async (tx) => {
      for (const lesson of lessons) {
        console.log(
          `[DEBUG] Updating lesson ${lesson.id} to order ${lesson.order}`,
        );
