...
export async function POST(request: NextRequest) {
  try {
    ...
    await db.insert(courseSchema.course).values({
      id: courseId,
      title: body.title,
    ...
    await db.insert(courseSchema.courseModule).values({
      id: defaultModuleId,
      title: "Module 1",
    ...
    if (
      ...
      const lessonValues = body.lessons.map(
        (lesson: LessonInput, index: number) => ({
          id: `lesson-${nanoid()}`,
