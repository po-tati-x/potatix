...
export function SlugEditor({ currentSlug, onUpdateSlug }: SlugEditorProps) {
  ...
  const [slugValue, setSlugValue] = useState<string>(currentSlug);
  ...
  const handleSave = async () => {
    ...
    // Delegate actual API call / duplication check to parent component
    try {
      await Promise.resolve(onUpdateSlug(slugValue));
      setIsEditing(false);
      setError(undefined);
    } catch (error_) {
      setError("Failed to update slug");
      console.error("Slug update error:", error_);
    }
  };
